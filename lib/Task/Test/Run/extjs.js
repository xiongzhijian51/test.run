		...
		methods : {
			...
			setPreloadFromTaskName : function (taskName) {
				...
				Joose.A.each(COMPONENTS[taskName], function (comp) {
					preload.push( /\.js$/.test(comp) ? comp : '../lib/' + comp.split('.').join('/') + '.js' )
				})
...
//			},
			...
			testEnd : function (test) {
				if (!this.isRunning() && !this.reported) {
					...
					var res = req.getText('/proven/' + this.browser + '/' + (this.isPassed() ? 'pass' : 'fail'))
                    
                    if (res == 'done') try {
                        new JooseX.SimpleRequest().getText('/done')
                    } catch (e) {}
                }
			}
